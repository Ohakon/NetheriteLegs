#> flower_position:get
### 実行点の位置の花/竹等の位置を計算する
# https://github.com/MCJE-Tech-Community/Datapack-WIki/tree/main
...
## 実行点の座標を取得
    ...
    execute store result score # _ run data modify storage flower_position: pos2 set from entity 4fe002bb-0-0-0-1 Pos
    # 前回と同位置なら終了
    execute if score # _ matches 0 as 4fe002bb-0-0-0-1 at @s run function flower_position:tp with storage flower_position: macro
    execute if score # _ matches 0 run return fail
    data modify storage _ pos set from storage flower_position: pos2
...
## 出力
    ...
    data modify storage flower_position: macro.x set from storage _ pos[0]
    data modify storage flower_position: macro.z set from storage _ pos[2]
    execute as 4fe002bb-0-0-0-1 at @s run function flower_position:tp with storage flower_position: macro
    # スコアホルダーの削除
